...
class Character: NSObject {
    ...
    func modifyLavaLightFlash() {
        ...
        while playingLavaLightFlash {
            ...
            usleep(useconds_t(0.015 * second)) // 15 ms
    ...
    func characterDirection(withPointOfView pointOfView: SCNNode?) -> SIMD3<Float> {
        ...
        if let pov = pointOfView {
            let point1 = pov.presentation.simdConvertPosition(SIMD3<Float>(controllerDir.x, 0.0, controllerDir.y), to: nil)
            let point2 = pov.presentation.simdConvertPosition(SIMD3<Float>.zero, to: nil)
